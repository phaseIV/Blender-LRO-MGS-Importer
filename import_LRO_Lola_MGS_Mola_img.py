...
bl_info = {
    ...
    "author": "Valter Battioli (ValterVB) / phaseIV",
    "version": (1, 1, 9),
    "blender": (2, 80, 0),
    "location": "3D window > Tool Shelf",
    "description": "Import DTM from LRO Lola and MGS Mola",
    "warning": "May consume a lot of memory",
    "wiki_url": "https://archive.blender.org/wiki/index.php/Extensions:2.6/Py/Scripts/Import-Export/NASA_IMG_Importer/",
    "tracker_url": "TBA",
...
#ver. 1.1.9: -Modified to work with Blender 2.8+ (phaseIV)
#************************************************************************
...
def update_fpath(self, context):
    global start_up
    start_up=False
    print('KYE', bpy.context.scene.fpath, bpy.path.abspath(bpy.context.scene.fpath))
    ReadLabel(bpy.path.abspath(bpy.context.scene.fpath))
    if Message != "":
        start_up=True
    else:
        typ = bpy.types.Scene
...
class Import(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        FileAndPath = bpy.path.abspath(bpy.context.scene.fpath)
        FileAndExt = os.path.splitext(FileAndPath)
        #Check for UNIX that is case sensitive
        ...
        print('*** End Smooth   ***')
        if TARGET_NAME == "MOON":
            add_material(ob_new, "Moon", 0.426, 0.426, 0.426)
        elif TARGET_NAME == "MARS":
            add_material(ob_new, "Mars", 0.426, 0.213, 0.136)
        print('*** FINISHED ***')
        return {'FINISHED'}
    ...
    def draw(self, context):
        ...
        if start_up:
            ...
            if bpy.context.scene.fpath != "":
                col = layout.column()
                split = col.split(align=True)
                split.label(text="1 Blender unit = " + str(bpy.context.scene.Scale) + RadiusUM)
            ...
            if VertNumbers > 3 and (RealLat(bpy.context.scene.FromLat) > RealLat(bpy.context.scene.ToLat)) and (RealLong(bpy.context.scene.FromLong) < RealLong(bpy.context.scene.ToLong)):  # If I have 4 or plus vertex I can import
                ...
                split.label(text=str(2 * math.pi * OFFSET / 360 / MAP_RESOLUTION) + " " + RadiusUM + "/pix")
                col = layout.column()
                split = col.split(align=True)
                split.label(text="Real Northernmost Lat.: " + str(RealLat(bpy.context.scene.FromLat)) + " deg")
                split.label(text="Real Southernmost Long.: " + str(RealLat(bpy.context.scene.ToLat)) + " deg")
                split = col.split(align=True)
